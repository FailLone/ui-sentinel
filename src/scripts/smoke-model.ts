...
async function main(){
  ...
  try{
    await worker.page.setContent('<html><body><h1>Visual smoke</h1><button style="margin:70px;padding:25px;background:#164ed6;color:white">Confirm purchase</button></body></html>')
    const vision=createVisionLocator(worker.page,{signal,beforeModelCall:()=>{if(++visionRequests>3)throw new Error('smoke vision request budget exhausted')}})
    const located=await vision.aiLocate('the blue Confirm purchase button')
    const hit=await worker.page.evaluate(({x,y})=>document.elementFromPoint(x,y)?.closest('button')?.textContent,{x:located.center[0],y:located.center[1]})
    if(hit!=='Confirm purchase')throw new Error('Vision coordinate did not hit the requested control')
    await mkdir('data/smoke',{recursive:true})
    await worker.page.screenshot({path:'data/smoke/model.png'})
    await writeFile('data/smoke/model.json',JSON.stringify({realModel:true,agentModel:config.agentModel,visionModel:config.visionModel,toolCalls,visionRequests,usage:result.usage??'unavailable',hit,at:new Date().toISOString()},null,2))
