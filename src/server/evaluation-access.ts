...
export function canCreateRun(authorization?: string) {
  return !lease || evaluationAuthorized(authorization)
}
export const evaluationRoutes = new Hono()
evaluationRoutes.use('/api/evaluation/*', async (c, next) => {
  if (!evaluationAuthorized(c.req.header('authorization')))
    return c.json({ error: 'unauthorized' }, 401)
  await next()
})
evaluationRoutes.post('/api/evaluation/lease', async (c) =>
  withAdmission(async () => {
    ...
    return c.json({
      lease,
      rules: getAllRules().map((r) => ({ id: r.id, revision: r.revision, category: r.category })),
    })
  }),
)
...
evaluationRoutes.post('/api/evaluation/reconcile', async (c) => {
  ...
  if (body.verified !== true || !body.reason?.trim())
    return c.json({ error: 'Explicit independent verification and reason required' }, 400)
  ...
  for (const row of rows.rows)
    await appendEvent(String(row.id), 'run:reconciled', {
      reason: body.reason,
      verifiedBy: 'private-controller',
    })
  return c.json({ acknowledged: true })
})

let admissionTail: Promise<unknown> = Promise.resolve()
export function withAdmission<T>(operation: () => Promise<T>): Promise<T> {
