...
export const transitionRuleSchema = z
  ...
  .strict()
export function validateRuleConfig(config: TransitionRuleConfig): void {
  transitionRuleSchema.parse(config)
}
const observationSchema = z.object({
  eventType: z.string(),
  fromState: z.string().optional(),
  toState: z.string().optional(),
  condition: z.enum(['state-reachable', 'element-visible', 'element-actionable']).optional(),
  startedAtMs: z.number().nonnegative(),
  observedUntilMs: z.number().nonnegative(),
  samples: z.array(
    z.object({
...
export function evaluateTransition(
  ...
  if (
    !observation ||
    observation.eventType !== config.trigger.eventType ||
    (config.trigger.fromState && observation.fromState !== config.trigger.fromState) ||
    (config.trigger.toState && observation.toState !== config.trigger.toState)
  )
    return 'unknown'
  if (
    !Number.isFinite(observation.startedAtMs) ||
    !Number.isFinite(observation.observedUntilMs) ||
  ...
  const samples = observation.samples
    .filter(
      (s) =>
        s.target === config.expectation.target &&
        s.atMs >= observation.startedAtMs &&
        s.atMs <= deadline,
    )
  ...
  if (
    samples.some(
      (s) =>
        s.value === true ||
        (config.expectation.condition === 'state-reachable' &&
          s.value === config.expectation.target),
    )
  )
    return 'pass'
  // Bounded continuous sampling is needed to assert the entire window was unavailable.
  if (
    observation.observedUntilMs < deadline ||
    !samples.length ||
    samples.some((s) => s.value !== false)
  )
    return 'unknown'
  if (
    samples[0]!.atMs - observation.startedAtMs > 250 ||
    deadline - samples.at(-1)!.atMs > 250 ||
    samples.some((s, i) => i > 0 && s.atMs - samples[i - 1]!.atMs > 500)
  )
    return 'unknown'
  return 'fail'
}
  ...
  return {
    ...
    async evaluate({ snapshot }) {
      ...
      return {
        ...
        actual: `${relevant.length} transition observations: ${verdict}`,
        evidenceRefs: relevant.flatMap((o) => [...o.evidenceRefs]),
        confidence: verdict === 'unknown' ? 0 : 0.95,
        details: { observations: relevant },
      }
    },
  }
}
