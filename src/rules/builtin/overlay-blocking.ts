...
export const overlayBlockingRule: Rule = {
  ...
  async evaluate({ snapshot }) {
    ...
    const partial = sampled.filter(
      (e) => e.hitSamples!.some((s) => s.relation === 'unrelated') && !blocked.includes(e),
    )
    const verdict = blocked.length
      ? 'fail'
      : sampled.length < targets.length
        ? 'unknown'
        : targets.length
          ? 'pass'
          : 'not-applicable'
    return {
      ...
      title: blocked.length
        ? 'Visible action intercepted at all sampled points'
        : 'Action hit testing',
      ...
      confidence: blocked.length ? 0.95 : 0.8,
      details: {
        blocked: blocked.length > 0,
        blockedTargets: blocked,
        partialTargets: partial,
        method: 'five-point-hit-test',
        originalViewport: snapshot.viewport,
      },
    }
  },
}
