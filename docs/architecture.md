...
2026-09-24 架构收敛决定：保留现有技术栈，采用下述有限职责分工。固定模型下的陌生布局 12/12、正式 minimum 18/18、获准规则复查 6/6 已通过；这是已测范围的选择，不是永久全局最优证明。完整对照、失败、费用与适用限制见[架构验证结果](../plans/architecture-convergence-results.md#最终架构决定)。
...
完整 Agent 继续负责开放探索、语义目标、要求适用性和未知问题。已有规则与原子调查是执行能力：一次调用可以完成登记、连续测量、有限谓词验证与证据保存，避免每个机械步骤都重新询问模型；没有规则并不会禁止探索。

原子调查默认启用，可用 `EXECUTION_ATOMIC_INVESTIGATION=0` 回退。可选 `EXECUTION_BLOCKER_REVIEW=1` 接入 Jev，只在已有支持性阻断证据、没有未解决工作/恢复控件且现场稳定时提出 observed-blocker 建议。它通过 OpenRouter Decisions API 的只读适配器调用，不是另一套浏览器 Agent 循环。与主 Agent 共享预算、取消和审计；结束前仍由执行器重验，其他回答或错误回到主 Agent。该能力默认关闭，需要显式配置 OpenRouter 凭据。

不采用通用 Jev 结束权限，也不整体更换 Stagehand/Browser Use：固定条件下的实际对照未满足质量要求。后台视觉分析继续默认关闭，页面操作仍串行；允许不可变证据上的独立分析，并不等于已经取得端到端并行收益。

完成报告从单一读事务生成，执行结束后用新连接核对终态及事件尾。记录不一致时显示完成未验证，隔离后续写任务，待业务状态核对后恢复。它保护证据与业务一致性，不表示原持久化异常根因已经查明。现有单机/业务适配、复杂页面快路径覆盖与长尾限制见验证结果。
