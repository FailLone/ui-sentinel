...
## 执行效率迭代：规则路由（2026-09-23，契约 13）

规则可声明 `routing` 元数据：版本、automatic/semantic-binding、依赖事件和可选业务触发类型。已知自动规则没有触发事件时跳过；缺元数据走原评价路径。学习规则尚无合法绑定事实时记录 unknown/待绑定，不运行无事实的重复评价，也不当作 pass 或不适用。

默认工作包最多给五条相关/待确认规则，保留总数、适用性计数与分页位置；`rules_search({query, offset})` 查询全部启用规则，`rule_details({ruleId})` 获取完整声明。当前公开事实已触发的学习规则，即使不在首页也计入待检查数量；结束时未验证的检查保留在覆盖账本。未知适配资格仍保持 unknown。

自动检查只在同一 Run、同一规则修订、已验证的页面事实版本和相同依赖事件下复用；未知依赖的旧规则、语义绑定检查不使用这层缓存。复用引用原始证据，规则修订、节点/页面变化或新业务事件使缓存失效。`EXECUTION_RULE_ROUTING=0` 可回到全规则评价与上下文；已有 bound-only 学习事实边界不变。
