...
**实现边界：** 当前事件和语义目标由 Agent/采集器提供，执行器尚无独立的通用重试资格校验器、操作实例绑定或完整的逐目标覆盖汇总。因此不能把文档中的资格约定当成已由代码强制执行的保证，也不能用一个入口的 pass 声称全部入口通过。当前三态 evaluator 对不匹配事件返回 unknown，设计层面的 skip 尚未在此模板实现。扩大到真实多目标业务前，必须验证资格识别、目标绑定、冷却/次数限制等反例；规则层合成用例不替代这些端到端验证。
