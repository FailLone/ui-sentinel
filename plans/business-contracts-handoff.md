...
| PR 或 bundle / bundle base | 未创建 PR、未 push（push 被会话权限拒绝，见「偏差」第 6 项）。已交付 bundle `ui-sentinel-business-contracts.bundle`（仓库根目录，**在最终 tip 重新生成**）；**base `1b377bb7d1bcbeb0ae607930ddc8506a486175ab`**（= `origin/main` 现有提交），head = `dev/business-contracts-export` 的 tip `e55ba66510cf8361c5dbbb39d5536d545c11921d`（`base..tip` 共 47 个提交）。`git bundle verify` 通过，并已在一个**只有该 base 的干净裸库**里实际 fetch 成功（tip 与 47 个提交均正确落地）——不只是静态校验 |
