...
| PR 或 bundle / bundle base | 未创建 PR、未 push（push 被会话权限拒绝，见「偏差」第 6 项）。已交付 bundle `ui-sentinel-business-contracts.bundle`（仓库根目录，**在最终 tip 重新生成**）。**base `1b377bb7d1bcbeb0ae607930ddc8506a486175ab`**（= `origin/main` 现有提交）。head **不在本文件写死**——它就是 bundle 自己的 `refs/heads/dev/business-contracts-export`，用 `git bundle list-heads <file>` 读取；因为每次提交（包括本记录自身）都会移动 tip，写死的 SHA 必然过期。`git bundle verify` 通过，并已在一个**只有该 base 的干净裸库**里实际 fetch 成功（tip 与全部 47 个提交正确落地）——不只是静态校验 |
...
冻结 f033992 的诊断（data/business-validation/2026-09-25T14-52-37-068Z）smoke+E0–E4 全过，$0.05291288。随后正式批次 data/business-formal/2026-09-25T14-56-32-809Z 实际执行8轮：6通过，2失败，37未运行；不是45轮通过。全部8轮停服审计通过，累计费用含诊断 $0.12773872。

发现并修复：未知业务结果的 `scope-covered` 结束未保留未验证路径；诊断构造了不完整 report，绕过覆盖字段检查；模型连续两次输出到上限未调用工具后，runner 将 `not-final` 的普通执行错误误判为持久化损坏并停止。新版本保留未知路径，诊断传完整报告，只有真实 `inconsistent`/证据损坏等才作为完整性停止。原失败原样保留。

另外明确公开业务契约中“业务重试资格”和“工作台入口可用性”是不同事实；原子调查返回后续可恢复/结束指引，不提高输出、请求、时间或费用上限。导出选择由私有请求审计取得，避免拿响应字段自证选择正确；产物内容包含对应数据集数据。

下一批可用费用上限从 $2 扣除上述全部历史支出，为 $1.87226128；新诊断及正式批次共享余额。


### 完整矩阵复验及收尾更正（2026-09-26）

2433b3f 的诊断 data/business-validation/2026-09-25T15-19-02-263Z 为6/6。正式批次 data/business-formal/2026-09-25T15-25-37-712Z 完整执行45轮：A15/15、B5/6、C18/18、D6/6，总44/45，不通过；四组停服审计均通过。唯一失败是 B/E2/2 已完成规则 fail 和全部缺陷证据后，模型连续两次耗尽4096输出，没有调用 finish。不是测量失败或规则未迁移。

更正：检查后指引统一复用，明确 fail 已解决该检查、pass 仍须实际恢复、unknown 不得当已完成；消除 Agent 上下文“一概只读”和允许重试的矛盾，分别显示创建余额与条件重试政策；唯一安全模型重试明确选择下一工具或显式 finish，不生成报告，不增加模型/时间/输出预算。执行器仍独立验证结束，未强制把模型错误转为成功，未扩大 Jev 收尾资格。

本批正式 accounted=$0.485128228（known=$0.469688428，2次未知费用保守预留$0.0154398），其诊断$0.08153276。并将 dev 报告的历史$0.04401488保守计入$2总预算；下一诊断/正式共享余额$1.159362212。此前所有失败仍保留。
