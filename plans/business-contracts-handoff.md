...
目前累计accounted **$2.673734774**，含dev报告及两批未知费用预留合计 **$0.042555**；已批准累计上限$3，余额 **$0.326265226**。上一完整批单独$0.423812928，剩余额度不足以稳妥安排新的完整矩阵。累计提高至$3.50的请求仍未获答复；所有付费进程已结束，没有在等待期间突破原上限。修复与记录已推送审查分支，main仍是766615b。

若用户确认累计上限$3.50，下一步使用同一冻结构建直接完整复验，无需再次重复已经通过的同构建诊断：

```sh
VALIDATION_MAX_COST_USD=0.878530320 pnpm validate:business -- --formal \
  --diagnostic-source data/business-validation/2026-09-26T15-54-34-852Z \
  --approved-source data/fixtures/approved-retry
```

该env是$3.50扣除诊断之前所有旧费用及B/C定向费用；formal再扣所引用诊断$0.052265094，实际可用$0.826265226，不重复授予预算。**此命令依赖尚待确认的$3.50上限，当前不得执行。** 若有普通质量失败继续保留全矩阵；完整45轮和四组停服审计全过后才标accepted、按用户授权合并main并push。
