...
if (args.some((a) => !['--minimum', '--minimum-only'].includes(a)) || args.length > 1)
  throw Error('Usage: pnpm validate:acceptance [--minimum | --minimum-only]')
...
try {
  ...
  const metadata = {
    ...
    agentProvider: process.env.VALIDATION_AGENT_PROVIDER ?? 'auto',
    visionProvider: process.env.VALIDATION_VISION_PROVIDER ?? 'auto',
  ...
  const smoke = await wait(
    launch('smoke', ['--import', 'tsx', 'scripts/cli/smoke-model.ts']),
    95000,
  )
  ...
  const { runEvaluation } = await import('../cli/evaluate.ts')
