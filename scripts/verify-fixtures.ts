...
const env = {
  ...
  ARENA_PORT: String(await port()),
  ARENA_API_PORT: String(await port()),
  ARENA_CONTROL_PORT: String(await port()),
...
try {
  const base = `http://127.0.0.1:${env.PORT}`,
    arena = `http://localhost:${env.ARENA_PORT}`
  let ready = false
  for (let i = 0; i < 100; i++) {
    try {
  ...
  const deny = await fetch(base + '/api/runs', {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ goal: 'inspect' }),
  })
  ...
  if (privateResponse.status !== 401)
    throw new Error('Private control must reject unauthenticated access')
  for (const path of ['/__control/state', '/src/server/state.ts', '/evaluation/private/answers.ts'])
    if ((await fetch(arena + path)).status !== 404)
      throw new Error('Private/source route exposed: ' + path)
  const { resetAndVerify } = await import('../evaluation/private/controller.ts')
  const checks = []
  for (const variant of ['C0', 'C1', 'C2', 'C3', 'C4', 'C5'] as const) {
    const result = await resetAndVerify(variant)
    checks.push({ variant, ...result })
    console.log(`${variant}: fixture verified`)
  }
  await mkdir('data/verification', { recursive: true })
  await writeFile(
    'data/verification/fixtures.json',
    JSON.stringify({ realModel: false, at: new Date().toISOString(), checks }, null, 2),
  )
  console.log(
    'Production boot, private isolation and all six fixtures passed; this is not an Agent/model evaluation.',
  )
} catch (error) {
  console.error(error)
  process.exitCode = 1
