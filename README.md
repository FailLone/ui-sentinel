...
业务契约阶段的最终门槛与修复记录以[交接记录](plans/business-contracts-handoff.md)为准。主 Agent 已修正证据引用、写入保护、事实时序、验收脚本和公开下载；完整复验还暴露模型收尾与重试额度指引问题，已补齐当前规则的有限语义审查及有效额度。当前632测试、32免费预检通过，最新修复构建仍须独立通过真实诊断和完整45轮。失败批次全部保留，不跨构建拼分。原批准来源由 Git fixture 携带，跨机器开发无需复制私有运行数据库。
