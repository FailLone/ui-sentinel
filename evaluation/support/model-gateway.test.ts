...
import { startGateway, AGENT_MODEL, REVIEW_MODEL } from './model-gateway.ts'
...
it('validation gateway refuses new requests once estimated spending exceeds the cap (fake upstream)', async () => {
...
it('validation gateway enforces actual request limits, model allowlist and credential isolation (fake upstream)', async () => {
...
it('validation gateway retains failed requests with unknown usage (fake upstream)', async () => {
...
it('enforces the accepted reasoning policy and output budget for validation', async () => {
  ...
  try {
    gateway.begin('accepted-policy', 1, 5000)
    const response = await fetch(gateway.url + '/chat/completions', {
      method: 'POST',
      headers: { authorization: `Bearer ${gateway.token}`, 'content-type': 'application/json' },
      body: JSON.stringify({
    ...
    expect(forwarded.reasoning).toEqual({ effort: 'low' })
