...
export const arenaUrl = () =>
  process.env.ARENA_URL ?? `http://localhost:${process.env.ARENA_PORT ?? 4173}`
...
export async function assertIdle(): Promise<void> {
  const response = await fetch(`http://localhost:${process.env.PORT ?? 4111}/api/health`, {
...
export async function resetAndVerify(variant: VariantId): Promise<Record<string, unknown>> {
  ...
  try {
    ...
    if (['C1', 'C2'].includes(variant))
      await page.getByTestId('checkout-overlay').waitFor({ state: 'visible' })
    const intercepted = await button.evaluate((el) => {
      const r = el.getBoundingClientRect()
      const hit = document.elementFromPoint(r.x + r.width / 2, r.y + r.height / 2)
      return !!hit && hit !== el && !el.contains(hit)
    })
    if (intercepted !== ['C1', 'C2'].includes(variant))
      throw new Error('Overlay hit-test fixture mismatch')
    if (variant === 'C2') {
      if (await page.getByRole('button', { name: 'Close', exact: true }).count())
        throw new Error('Unexpected close path')
    ...
    if (variant === 'C3') {
      if ((await button.innerText()) !== 'Complete Purchase')
        throw new Error('Renamed button missing')
      const bounds = await button.boundingBox(),
        back = await page.getByRole('button', { name: 'Back', exact: true }).boundingBox()
      if (
        !bounds ||
        !back ||
        bounds.x <= back.x ||
        ...
        throw new Error('Moved button must remain reasonably visible')
    }
    await button.click()
    ...
    if (
      ...
      !(await page.locator('.payment-result').innerText()).includes(order.id)
    )
      throw new Error('Backend and visible result mismatch')
    if (variant === 'C4' || variant === 'C5') {
      const retry = page.getByTestId('retry-button')
      const start = Date.now()
      do {
        if ((await retry.isEnabled()) !== (variant === 'C4'))
          throw new Error('Retry availability fixture mismatch')
    ...
    return {
      ...
      orderVisible: true,
      retryWindowVerified: ['C4', 'C5'].includes(variant),
    }
  } finally {
    await browser.close()
    ...
    if (clean.orderCount !== 0 || clean.cartSize !== 0)
      throw new Error('Reset did not clear verification side effects')
  }
}
